...
const Chat = () => {
  ...
  const {
    ...
    onError: (error: any) => {
      ...
      if (error.message?.includes('quota') || error.message?.includes('exceeded') || error.message?.includes('429')) {
        ...
        toast.error('⚠️ API Quota Exhausted! Free Gemini API limit reached. Please contact Iván directly or use preset questions. Thank you for understanding! 🙏', {
        ...
        try {
          append({
            role: 'assistant',
            content: '⚠️ **API Quota Exhausted**\n\nFree Gemini API limit reached. Please contact Iván directly or use preset questions below.',
          });
        } catch (appendError) {
  ...
  return (
    <div className="relative h-screen overflow-hidden">
      ...
      <div
        className="fixed top-0 right-0 left-0 z-50 bg-gradient-to-b from-background via-background/80 to-transparent pointer-events-none"
      >
        <div
          className={`transition-all duration-300 ease-in-out pointer-events-auto ${hasActiveTool ? 'pt-6 pb-0' : 'py-6'}`}
        >
