...
const AvailabilityCard = ({ data }: AvailabilityCardProps) => {
  ...
  const avatarSrc = config.personal.avatar || '/profile.jpeg';
  const seeking = config.internship?.seeking ?? false;
  const availabilityText = data?.availability
    || config.internship?.availability
    || (seeking ? '✅ Available for immediate start' : '❎ Not currently seeking opportunities');
  const locationText = data?.preferences?.location
    || config.internship?.preferredLocation
    ...
    >
      ...
      <div className="mb-6 flex flex-col items-center sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              {seeking ? 'Available for Opportunities' : 'Not currently seeking'}
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 flex flex-col items-center gap-2 sm:mt-0 sm:items-end">
          {seeking ? (
            ...
            <span className="flex items-center gap-1 rounded-full border border-zinc-400 px-3 py-0.5 text-sm font-medium text-zinc-600">
              <span className="relative inline-flex h-2 w-2 rounded-full bg-zinc-500"></span>
              Not Seeking
            </span>
          )}
          <p className="text-xs text-muted-foreground text-center sm:text-right">
            {seeking ? 'Open to full-time or contract roles' : 'Open to networking and collaboration'}
          </p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-blue-600 dark:text-blue-400 font-semibold">
              {seeking ? (config.internship?.focusAreas?.join(', ') || 'Full-time roles, Internships & Contract work') : '—'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              {config.internship?.startDate ? `Start Date: ${config.internship.startDate}` : availabilityText}
            </p>
          </div>
        </div>
        <div className="flex items-start gap-3">
      ...
      <div className="mt-10">
        ...
        <p className="text-foreground text-sm">
          {config.internship?.workStyle || "Focused on driving AI adoption and digital transformation through data-driven engineering solutions."}
        </p>
      </div>
      ...
      <div className="mt-8">
        ...
        <p className="text-foreground text-sm">
          {config.internship?.goals || "Leveraging AI to improve efficiency, decision-making, and innovation in real-world applications."}
        </p>
      </div>
